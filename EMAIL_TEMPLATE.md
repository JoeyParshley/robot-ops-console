...
- **TypeScript-first development** for type safety in safety-critical operations
- **Custom React hooks** for reusable business logic (real-time telemetry, robot controls)
- **Modern state management** using React Context API
- **Electron desktop deployment** with security best practices
- **Test-driven development** with comprehensive test coverage

I used GitHub Projects to manage the weekend sprint, and the codebase is fully documented. There's also an **Executive Summary** in the repo (`EXECUTIVE_SUMMARY.md`) that provides a quick 2-minute overview if you'd like to share it with the hiring manager.

**To Run Locally:**
1. Clone the repo
2. Run `npm install && cd server && npm install && cd ..`
3. Start the telemetry simulator: `cd server && npm start`
4. In another terminal, run: `npm run electron:dev`
...
- **Include Electron App?** Yes, mention it as optional. Most hiring managers prefer exploring code, but offering a pre-built binary shows you're considerate of their time.
- **Format**: Plain text email is fine. If you want to be more formal, you can format it as HTML, but plain text is more personal and less likely to be filtered.
- **Salary Negotiation**: The email acknowledges the discussion while remaining positive and flexible. This shows you're reasonable and focused on the opportunity.
- **Timing**: Send this 1-2 days before your interview so they have time to review if interested.
